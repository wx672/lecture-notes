...
* UEFI
现在（2026/03/24）的电脑，在主板上几乎都用UEFI取代了传统的BIOS。关于UEFI设置，我的经验是：
...
- 如果在下面的安装过程中（硬盘分区的时候）看不到硬盘，那么你需要在UEFI
  设置里找到 “Intel Rapid Storage Technology (Intel RST)”，把它设置为 “AHCI”。
...
5. 只要两个分区 =swap= 和 =/= ，换言之, *不要 =/boot, /home, /var, ...= 等分
   区* 。
...
1. 先准备好一个安装盘（LiveUSB）
   1) *下载链接:* [[https://cdimage.debian.org/cdimage/release/current/amd64/bt-cd/][https://cdimage.debian.org/cdimage/release/current/amd64/bt-cd/]]
      ...
      #+begin_example
      - debian-13.1.0-amd64-netinst.iso.torrent     <-- 只要这一个
      - debian-edu-13.1.0-amd64-netinst.iso.torrent
      - debian-mac-13.1.0-amd64-netinst.iso.torrent
      #+end_example
      ...
      #+begin_src sh
        aria2c --no-conf debian-13.1.0-amd64-netinst.iso.torrent
        # 文件名太长？那么就：
   ...
   2) *制作U盘:* 在Debian/Ubuntu平台上制作启动U盘非常简单，敲个命令就行了：
      #+BEGIN_SRC sh
        sudo cp debian-13.1.0-amd64-netinst.iso /dev/sdX  # 把 sdX 换成 sdb 或者 sdc。
                                                          # 用 lsblk 命令看一眼，
                                                          # 就知道是 b 还是 c 了。
        # 文件名太长？那么就用<TAB>键帮你补全：
...
2. 拔掉网线，从U盘重启系统，开始安装。大概半个小时，一个最小系统就装
   好了。「拔掉网线」只是我的个人习惯，并不是必须的。联网安装的话，可能会遭遇若干问题：
   1) 如果网络不畅，安装过程会很慢，甚至失败；
   2) 时髦的笔记本往往不带有线网口，如果选择联网安装，安装过程中就要涉及安装网卡驱动、输
      入无线密码等步骤，麻烦多多；
   3) 联网安装，系统还会提示你选择镜像源、选择要安装的“全家桶”，这些貌似友好的功能选项，在
      我看弊大于利，只能增加入门者的困扰。
3. 完事后，拔掉U盘，重启系统。
4. 【注意】
   + 再次强调, *不要选择中文语言环境*! 因为后面的安装配置工作都是在非图形环境下进行，
     采用中文的话，你很可能会遭遇乱码。
...
2. 联网。当然要先插好网线。如果你的笔记本比较新潮，没有有线网口，那么可以试试下面两个办法：
   ...
   #+begin_src sh
     ip a          #查看网卡是否已经有IP地址了。如果没有，
     sudo dhclient #自动获得IP地址
     # 如果遭遇command not found（最小系统没装好？），就
     sudo dhcpcd
   #+end_src
...
4. 运行
   ...
   网络顺畅的话，半小时就能完事。不顺畅的话……把网络搞顺畅了再说吧。
...
办法很多：
1. 用Android手机的USB Tethering功能。以我自己的手机系统为例（crDroid 12.8/Android 16），
   很简单，
   1) 用USB线连接手机和电脑；
   ...
   3) 在电脑上，敲命令 =ip a= 应该能看到一块有线网卡。比如，
      ...
      #+END_EXAMPLE
      从上面的屏幕输出信息可以看到，这块有线网卡的名字是 =enp2s0f4u2= 。然后，敲下面这条命令：
      #+BEGIN_SRC sh
...
3. 当然，也可以试试能否让内置无线网卡工作，但毕竟我还没有亲自尝试过，所以只能先给出
   一些想法：
   - 可以试试把内核和相应固件从稳定版更新到测试版（testing）。在没有网
     络连接的情况下，显然这需要我们另找办法下载，并手动安装一些软件包，
     包括：
