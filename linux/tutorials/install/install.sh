...
PKG_REC="apt-file bat catdoc cht.sh convmv default-jre dialog dict dict-foldoc dict-gcide dict-jargon dict-vera dict-wn dosfstools elpa-pdf-tools-server emacs emacs-common-non-dfsg eza fd-find ffmpeg firmware-linux-free fzf g++ gawk gcc gdb global hunspell imagemagick ipcalc keynav kmscon lf libnotify-bin libreoffice-calc libreoffice-impress libreoffice-writer libreoffice-qt6 libtext-csv-xs-perl lloconv lshw make manpages-posix manpages-posix-dev mdp mpv mupdf nala ncat netcat-openbsd nmap notification-daemon nsxiv ntfs-3g org-mode-doc pandoc parted poppler-utils profile-sync-daemon proxychains4 pulsemixer pv pqiv qutebrowser qt6ct ripgrep rofi rsync tcpdump tlp tmate tpp ttyrec unar universal-ctags visidata vivid wamerican-insane wireless-regdb xbanish xlsx2csv zathura zoxide"
...
deb https://mirrors.qvq.net.cn/debian/  sid main contrib non-free non-free-firmware
...
{ 
	...
	until $WGET -nc -P $DIR $BASEURL/debian-install/misc/{stterm.deb,dwm.deb,elpa.tgz,tmux-plugins.tgz,FiraCodeNerdFont.tgz,cn/dict-cn.tgz,media-test.mp4,starship}; do
