...
apt_setup(){
  ...
  # sources.list
	colorEcho "$INFO" "Populating /etc/apt/sources.list.d/debian.sources ..."
...
more_pkgs(){
	...
	mkdir ~/.emacs.d
	ln -sf /usr/local/share/emacs/elpa ~/.emacs.d/elpa
	...
	do
		ln -sf /usr/local/share/$d ~/.config/$d
	done
	
	ln -sf /usr/local/share/picom/picom.conf ~/.config/picom.conf
	ln -sf /usr/local/share/starship/starship.toml ~/.config/starship.toml
}
...
EOF
	...
	mkdir -p .local/share .cache/emacs/etc/yasnippet .config
