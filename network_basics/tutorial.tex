...
\begin{frame}{A \(\Rightarrow\) E}
  ...
  \begin{minipage}{.45\linewidth}
    ...
    \end{iblock}
  \end{minipage}
  \mode<beamer>{\begin{iblock}{}
      \begin{center}
        \includegraphics[width=.5\linewidth]{indirect-routing}
      \end{center}
    \end{iblock}}
  % \mode<beamer>{
  %   \begin{tikzpicture}[remember picture, overlay]%
...
\begin{frame}{Network bits?}
  ...
  \begin{iblock}{Prefix --- A faster way to decide network bits}
    \centering \mode<beamer>{ \includegraphics[width=.8\textwidth]{prefix2} }%
...
\begin{frame}{Mail vs. E-mail}
  \begin{center}
    \includegraphics[width=.47\textwidth]{surfacemailing}\hfill
    \includegraphics[width=.5\textwidth]{router4-tikz}
  \end{center}
\end{frame}
...
\begin{frame}{Example: VLSM}
  \begin{minipage}{.5\linewidth}
    \includegraphics[width=1.2\textwidth]{subnetting-example}    \\[3ex]
  ...
  \end{minipage}\hfill
  \begin{minipage}{.5\linewidth}
    \includegraphics[width=.9\textwidth]{hierarchy-subnet}
  \end{minipage}
\end{frame}
...
\begin{frame}{Example}
  \vspace*{-1.5em}
  \includegraphics[scale=.6]{subnet-194}
...
%  \vspace*{-1em}
  ...
  \end{minipage}\hfill
  \begin{minipage}{.55\linewidth}
    \includegraphics[width=\textwidth]{route-aggregation}
  \end{minipage}
\end{frame}
...
\begin{frame}{More Examples}\small
  ...
  \item[\$] \cmd{sudo iptables -A FORWARD -p tcp --syn -m limit --limit 1/s -j ACCEPT}
  \item[]
  \item[\$] \cmd{\# Furtive port scanner:}
  \item[\$] \cmd{sudo iptables -A FORWARD -p tcp --tcp-flags SYN,ACK,FIN,RST RST -m limit
      --limit 1/s -j ACCEPT}
  \item[]
  \item[\$] \cmd{\# Ping of death:}
  \item[\$] \cmd{sudo iptables -A FORWARD -p icmp --icmp-type echo-request -m limit
      --limit 1/s -j ACCEPT}
  \end{itemize}
\end{frame}
