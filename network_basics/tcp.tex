...
\begin{frame}{\scalebox{3}{\ding{37}}\quad \raisebox{9pt}{vs.}\quad \scalebox{3}{\ding{41}}}
  \begin{minipage}[t]{.47\linewidth}
    \begin{iblock}{Circuit switching (\ding{37})}
      ...
      \item[\Bad] wastes bandwidth if traffic is ``bursty''
      \item[\Bad] connection setup adds delay
      \item[\Bad] recovery from failure is slow
      \end{itemize}
    \end{iblock}
  \end{minipage}\hfill
  \begin{minipage}[t]{.47\linewidth}
    \begin{iblock}{Packet switching (\ding{41})}
      \begin{itemize}
      \item[\Bad] no guaranteed performance
      \item[\Bad] header overhead per packet
      \item[\Bad] queues and queuing delay
...
\begin{frame}
  ...
  \begin{iblock}{QoS on data link layer or IP layer?}
    \begin{itemize}
    \item[\Bad] Efficiency
    \item[\Bad] Upgrade all the routers
    \end{itemize}
  \end{iblock}
\end{minipage}\hfill
  \begin{minipage}[t]{.5\linewidth}
...
\begin{frame}{SYN-flood}
  ...
  \item[\$] \cmd{nc -l 3333}
  \item[\$] \cmd{sudo iptables -A OUTPUT -p tcp -m tcp --tcp-flags RST RST -j DROP}
  \item[\$] \cmd{sudo hping3 -I lo -S -c 5 -p 3333 127.0.0.1}
...
\begin{frame}{\texttt{netstat}}
  ...
  \begin{block}{\texttt{ss} --- a \texttt{netstat} replacement}
    \begin{itemize}
    \item[\$] \cmd{ss -4ant "( sport = 3333 or dport = 3333 )"}
    \item[\$] \cmd{man ss}
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{UDP Example}
  ...
  \item[T2:\,\$] \cmd{nc -4u localhost 3333 \#client}
  \item[T3:\,\$] \cmd{echo hello > /dev/udp/127.0.0.1/3333 \#client}
  \item[T4:\,\$] \cmd{sudo tcpdump -ilo udp port 3333}
  \item[T5:\,\$] \cmd{watch -t -n.1 'ss -4anu "( sport == 3333 or dport == 3333 )"'}
  \end{itemize}
\end{frame}
