...
\begin{tikzpicture}[auto,node distance=15pt,%
  ...
  \node (chrome1)       [left=of local,chrome] {\includegraphics[width=10pt]{gchrome}};%
  \node (chrome2) [above left=of local,chrome] {\includegraphics[width=10pt]{gchrome}};%
  \node (chrome3) [below left=of local,chrome] {\includegraphics[width=10pt]{gchrome}};%

  \node [server node, right=130pt of local] (almond) {almond.nuts.com};
  ...
  (almond.184) edge ["plant.nuts.com NS pack.plant.nuts.com",swap, orange] (local.354)%

  (local.60) edge ["sale.plant.nuts.com",bend left=5, violet!70] (root.177)%
  (root.182) edge ["nuts.com NS almond.nuts.com",swap,bend right=5, blue!70] (local.45)%

  (local.320) edge ["sale.plant.nuts.com",bend right=5, violet!70] (pack.177)%
  (pack.184)  edge ["sale.plant.nuts.com A 172.16.6.4",swap,bend left=5, Green] (local.305)%
...
\begin{tikzpicture}[auto,%
  ...
  \begin{scope}[x={(image.south east)},y={(image.north west)}]%
    ...
    \node (req) at (.82,.65) {Request line};%
    \node (header) [below=.32 of req.west,anchor=west]{Header lines};%
    \node (bracket) [inner sep=0,left=of header] {$ \left\} \rule{0pt}{15mm}\right. $};%
...
\begin{tikzpicture}[auto,node distance=0pt,%
  ...
  server/.style = {rounded corners, fill=black!30, inner sep=2pt, font=\scriptsize},%
  shadow/.style ={minimum height=4pt, opacity=0},
  ]%
  ...
  \node (dnsserver) [server,minimum width=13mm,above=2cm of dns.north west,anchor=south west] {DNS Server};%
  \node (webserver) [server,minimum width=37mm,right=of dnsserver.east,anchor=west] {Web Server};%

  \node (dns1) [shadow,above=2cm of dns.north west,anchor=south west] {\rule{13mm}{0pt}};
  \node (connect1) [shadow,right=of dns1]{\rule{10mm}{0pt}};
  \node (send1) [shadow,right=of connect1]{\rule{6mm}{0pt}};
  \node (wait1) [shadow,right=of send1]{\rule{6mm}{0pt}};
  \node (load1) [shadow,right=of wait1]{\rule{15mm}{0pt}};

  \path%
  (dns.north west) edge ["DNS query",violet] (dnsserver.260)%
  (dnsserver.280)  edge ["IP address",violet] (dns.north east)%
  (connect.north west) edge ["SYN",orange] (connect1.250)%webserver.185
  (connect1.250) edge ["SYN/ACK",orange] (connect.north)%webserver.185
  (connect.north) edge ["ACK",orange] (connect1.south east)%webserver.187
  (send.north west) edge ["HTTP request",Green] (send1.south)%
  (wait1.south) edge ["HTTP response"',"1\textsuperscript{st} segment" near start,red!70] (wait.north east)%
  (load1.south west) edge ["PSH"',"2\textsuperscript{nd} segment" near start,blue!50] (load.172)%
  (load1.190) edge ["HTTP\_Continue"',"3\textsuperscript{rd} segment" near start,blue!50] (load.150)%
  (load.150) edge ["ACK" near end,blue!50] (load1.290)%
  (load1.350) edge ["FIN",black!60] (load.north east);%
\end{tikzpicture}  }
...
\begin{tikzpicture}[auto,node distance=0pt,%
  ...
  server/.style = {rounded corners, fill=black!30, inner sep=2pt,font=\scriptsize},%
  shadow/.style ={minimum height=4pt},
  ]%
  ...
  right=of dns] (connect) {\rule{10mm}{4pt}};

  \node [color=Green, label={[Green]below:{\scriptsize Send}},
  right=of connect] (send) {\rule{6mm}{4pt}};

  \node [color=red!70, label={[red!70]below:{\scriptsize Wait}},
  right=of send] (wait) {\rule{6mm}{4pt}};

  \node [color=blue!50, label={[blue!50]below:{\scriptsize Load}},
  right=of wait] (load) {\rule{10mm}{4pt}};

  \node [color=Green, label={[Green]below:{\scriptsize Send}},
  right=of load] (send2) {\rule{6mm}{4pt}};

  \node [color=red!70, label={[red!70]below:{\scriptsize Wait}},
  right=of send2] (wait2) {\rule{6mm}{4pt}};

  \node [color=blue!50, label={[blue!50]below:{\scriptsize Load}},
  right=of wait2] (load2) {\rule{10mm}{4pt}};
  
  \node (dnsserver) [above=2cm of dns.north west,anchor=south west,server,minimum width=13mm] {DNS Server};%
  \node (webserver) [right=of dnsserver.east,anchor=west,server,minimum width=55mm] {Web Server};%

  \node (dns1) [shadow,above=2cm of dns.north west,anchor=south west] {\rule{13mm}{0pt}};
  \node (connect1) [shadow,right=of dns1]{\rule{10mm}{0pt}};
  \node (send1) [shadow,right=of connect1]{\rule{6mm}{0pt}};
  \node (wait1) [shadow,right=of send1]{\rule{6mm}{0pt}};
  \node (load1) [shadow,right=of wait1]{\rule{10mm}{0pt}};
  \node (send3) [shadow,right=of load1]{\rule{6mm}{0pt}};
  \node (wait3) [shadow,right=of send3]{\rule{6mm}{0pt}};
  \node (load3) [shadow,right=of wait3]{\rule{10mm}{0pt}};

  \path%
  ($(dns.south west)+(0,-4mm)$) edge [thin, -] ($(dns.south west)+(0,-6mm)$)
  ($(load.south east)+(0,-4mm)$) edge [thin, -] ($(load.south east)+(0,-6mm)$)
  ($(load2.south east)+(0,-4mm)$) edge [thin, -] ($(load2.south east)+(0,-6mm)$)
  ($(dns.south west)+(0,-5mm)$) edge ["Request I",-,thin] ($(load.south east)+(0,-5mm)$)
  ($(send2.south west)+(0,-5mm)$) edge ["Request 2",-,thin] ($(load2.south east)+(0,-5mm)$)
  (dns.north west) edge ["DNS query",violet] (dnsserver.260)%
  (dnsserver.280)  edge ["IP address",violet] (dns.north east)%
  (connect.north west) edge ["SYN",orange] (connect1.250)%
  (connect1.250) edge ["SYN/ACK",orange] (connect.north)%
  (connect.north) edge ["ACK" near end,orange] (connect1.south east)%
  (send.north west) edge ["HTTP request",Green] (send1.south)%
  (wait1.south) edge ["HTTP response",red!70] (wait.north east)%
  (wait1.south east) edge [red!70] (load.110)%
  (load1.south) edge [red!70] (load.north east)%
  (send2.north west) edge [Green] (send3.south)%
  (wait3.south) edge [red!70] (wait2.north east)%
  (wait3.south east) edge [red!70] (load2.110)%
  (load3.south) edge [red!70] (load2.north east);
\end{tikzpicture}  }
