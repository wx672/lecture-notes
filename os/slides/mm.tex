...
\begin{frame}{Memory Management}
  ...
  \end{description}
  \begin{iblock}{In real world \ldots}
    \begin{center}
...
\begin{frame}{Basic Memory Management}{Real Mode}
  \begin{iblock}{In the old days \ldots }
    \begin{itemize}
...
\begin{frame}{ prefers paging to segmentation}
  ...
  \end{itemize}
  \end{iblock}
  The  2.6 uses segmentation only when required by the 80x86 architecture.
\end{frame}
...
\begin{frame}{Case Study: \intel Pentium}{Segmentation With Paging}
  \begin{center}
...
\begin{frame}{The Four Main  Segments}
  \begin{iblock}{Every process in  has these 4 segments}
    \begin{center}
...
\begin{frame}{\intel Pentium (x86) Paging}%{Linear Address $\Rightarrow$ Physical Address}
  \begin{minipage}{.4\textwidth}
...
% \begin{frame}{\intel x86\_64 Paging}
%   \begin{center}
%     \includegraphics[width=\textwidth]{intel-4-level-paging}
%   \end{center}
%   \begin{block}{Paging In }
%     4-level paging for both 32-bit and 64-bit
%   \end{block}
% \end{frame}

\begin{frame}{Paging In }{4-level paging for both 32-bit and 64-bit}
  \centering%
...
\begin{frame}{LRU Implementations}
  \begin{iblock}{\alert{Counters}: record the time of the last reference
    ...
    \item[\textcolor{red}{🙁}] require a \alert{search} of the page table to find the LRU page
    \item[\textcolor{red}{🙁}] update time-of-use field in the page table \alert{every memory reference!}
    \end{itemize}
  \end{iblock}
  \begin{iblock}{\alert{Stack}: keep a linked list (stack) of pages}
    ...
    \item whenever a page is referenced, it's removed from the stack and put on the top
      \begin{itemize}
      \item[\textcolor{red}{🙁}] update this list \alert{every memory reference!}
      \end{itemize}
    \end{itemize}
  \end{iblock}
\end{frame}
