...
\begin{frame}{CPU Switch From Process To Process}
  \mode<beamer>{\centering\includegraphics[height=.9\textheight]{cpu-switch} }%
  \mode<article>{\centering\includegraphics[width=.7\textwidth]{cpu-switch} }
\end{frame}
...
\begin{frame}{A Multi-threaded Web Server}\label{webserver}
  ...
  \end{center}
  \begin{center}
    \mode<beamer>{ \includegraphics[width=.75\textwidth]{mos-figs-2-11} }%
...
\begin{frame}{Why Having a Kind of Process Within a Process?}{}
  ...
  \item[🙂] Economy --- Threads are cheap!
    \begin{itemize}
    \item[🙂] Cheap to create --- only need a stack and storage for
      registers
    \item[🙂] Use very little resources --- don't need new address
      space, global data, program code, or OS resources
    \item[🙂] switches are fast --- only have to save/restore PC, SP, and
      registers
    \end{itemize}
...
\begin{frame}{Pthreads}
  ...
  \begin{block}{How to use pthread?}
    ...
    \item[] \CMD{gcc thread1.c -o thread1 -pthread}
    \item[] \CMD{./thread1}
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Pthreads}{Example 2}
  \begin{center}
    \mode<beamer>{ \includegraphics[height=.88\textheight]{thread3} }%
...
\begin{frame}<beamer>{Hybrid Implementations}{Combine the advantages of two}
  \centering\includegraphics[width=.8\textwidth]{mos-figs-2-14}
\end{frame}
...
\begin{frame}{The \texttt{clone()} System Call}
  \begin{center}
    \mode<beamer>{ \includegraphics[width=.8\textwidth]{clone-prototype-c} }%
...
\begin{frame}{Producer-Consumer Problem}
  ...
  \mode<article>{ \includegraphics[width=.4\textwidth]{ipc} }
\end{frame}
...
\begin{frame}{Producer-Consumer Problem}{--- Bounded-Buffer Problem (Circular Array)}
  ...
  \begin{minipage}{.45\linewidth}
    ...
    \mode<article>{ \includegraphics[width=.7\textwidth]{circular} }
  \end{minipage}
...
\begin{frame}{Mutual Exclusion With Busy Waiting}{Strict Alternation}
  ...
  \end{center}
  \begin{block}{🙁: Violates \hyperlink{cond}{condition-2}}
    \begin{itemize}
...
\begin{frame}{Unix GUI}
  \begin{tikzpicture}[remember picture, overlay]
    \node at (current page.center)%
    {%
      \includegraphics[width=\paperwidth]{Xwin.excalidraw}
    };
  \end{tikzpicture}%
\end{frame}
...
\begin{frame}{Sockets}
  ...
  \item[Domain] Determines address format and the range of
    ...
    \begin{center}
      ...
      \end{tblr}
    \end{center}
  \item[Type] \texttt{SOCK\_STREAM} (🕿), \texttt{SOCK\_DGRAM} (🖃)
...
\begin{frame}{Scheduling In Batch Systems}{First-Come First-Served}
  ...
  \item[🙂] nonpreemptive
  \item[🙂] simple
  \item[\textcolor{red}{🙁}] also has a disadvantage
    \begin{itemize}
...
\begin{frame}{Scheduling In Interactive Systems}{Round-Robin
  ...
  \item How long shoud the quantum be?
    \begin{itemize}
    \item[\textcolor{red}{🙁}] too short --- too many process switches, lower CPU efficiency;
    \item[\textcolor{red}{🙁}] too long --- poor response to short interactive requests;
...
\begin{frame}{Problems With The Pre-2.6 Scheduler}
  ...
  \item a single runqueue for all processors
    ...
    \item[\textcolor{red}{🙁}] bad for CPU caches, when a task is
      rescheduled from one CPU to another
    \end{itemize}
...
\begin{frame}{Deadlock Avoidance}{Mission Impossible}
  \begin{minipage}{.6\linewidth}
    \begin{block}{In practice}
      \begin{itemize}
      \item[\textcolor{red}{🙁}] processes rarely know in advance their max future resource needs;
      \item[\textcolor{red}{🙁}] the number of processes is not fixed;
      \item[\textcolor{red}{🙁}] the number of available resources is not fixed.
      \end{itemize}
    \end{block}
  \end{minipage}\qquad
  \begin{minipage}{.3\linewidth}
...
\begin{frame}{Deadlock Prevention}{Break The Four Conditions}
  \begin{block}{Attacking the Mutual Exclusion Condition}
    ...
    \item[\textcolor{red}{🙁}] Not always possible
      \begin{itemize}
...
\begin{frame}
  \begin{block}{Attacking the No Preemption Condition}
    ...
    \end{itemize}
  \end{block}
  \textcolor{red}{🙁}: Low resource utilization; starvation possible
\end{frame}
...
\begin{frame}
  ...
  % \end{center}
  \begin{itemize}
    \item[\textcolor{red}{🙁}] It's hard to find an ordering that satisfies everyone.
  \end{itemize}
\end{frame}
...
\begin{frame}{The Ostrich Algorithm}
  \begin{minipage}{.69\linewidth}
    ...
    \item It's a trade off
      \begin{itemize}
      \item[] convenience vs. correctness
      \end{itemize}
    \end{itemize}
  \end{minipage}\hfill
  \begin{minipage}{.29\linewidth}
